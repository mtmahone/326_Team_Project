...
//you can empty this out if need be.
var db = {
/*
  'matt'  : user('matt', 'ttam', "", true, []),
  'chris': user('chris', 'sirhc', "", true, []),
  'admin': user('admin', 'nimad', "", true, []),
  'qiwen': user('qiwen', 'newiq', "", true, []),
  'charlie': user('charlie', 'eilrahc', "", true, []),
  'senh': user('senh', 'hnes', "", true, [])
  */
  //addon additional test users here. this is because of the way UID's are assigned, and managed in the db. db reset may be necessary.
};
...
function SyncDB(databaseArrayToSync){
  ...
  for(var i in databaseArrayToSync){
    ...
    //run update of the stocks field.
    /*Not Working
    User.findOne({ name: databaseArrayToSync[i].name }, function (err, doc){
       if(err){
         console.log('an error has occurred '+err);
       }
       doc.stocks = databaseArrayToSync[i].stocks;
       doc.save();
    });
    //end update of stocks field
    */
    User.update({name: databaseArrayToSync[i].name}, {stocks: databaseArrayToSync[i].stocks}, 
    function(err, numberAffected, rawResponse) {
    //handle it
    console.log("There was an error? (should be null) "+err);
    console.log("Number Affected by update (should be 1) "+numberAffected);
    })
...
console.log("In-memory database just synced, contains the following elements");
console.log(db);
//take care here to ensure UID is at the end of the list.  
}
SyncDB(db);
...
//TO COMPLETE!!!
/* use the mongoose library to create a user in the add function, and have that user float up to the DB, sync it back to the in-memory db */
...
exports.add = (u, cb) => {
  ...
  */
  ...
  if(!userAlreadyExistsFlag){
    ...
    u.stocks = []; //if not initialized, you may be unable to add to watchlist.
  ...
  */
  ...
  //everything worked
  //KEYYYY TO HAVE UPDATES SENT TO DATABASE!
  SyncDB(db);
  cb(undefined,u);
