...
var team = [
	...
	member('cwongwajarachot','Charlie','Wongwajarachot','senior'),
	member('swang','Senh','Wang','senior'),
	member('cafonoso','Chris','Afonso','senior'),
	member('mmahoney','Matt','Mahoney','senior')
...
function one(user) {
    ...
    if (typeof user == 'string'){
        if(find(user)!= null){
            //console.log('user can be find')
            return result(true,'team member found',[copy(find(user))]);
            //console.log(result)
        }
        else return result(false,'team member not found',[]);
    }
	else return result(false,'please type a username',[]);
}
